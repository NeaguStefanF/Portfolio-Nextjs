...
const ProjectCard = ({ imgUrl, title, description, gitUrl, previewUrl }) => {
  ...
  return (
    <div>
      ...
      >
        <div
          className="overlay items-center justify-center gap-4 absolute
         top-0 left-0 w-full h-full bg-[#181818] bg-opacity-0 
         hidden group-hover:flex group-hover:bg-opacity-80 transition-all
          duration-500"
        >
          <Link
            href={gitUrl}
      ...
      <div className="text-white rounded-b-xl bg-[#181818] py-6 px-4 ">
        <div className="flex items-center justify-between">
          ...
          <button
            className="px-2 py-1 bg-[#333] text-white rounded-full
             w-8 h-8"
